...
class LSTMEncoder(nn.Module):
    def __init__(self, cfg):
        ...
        self.encoder_lstm = nn.LSTM(self.embed_size, self.hidden_size // 2, bidirectional=True, batch_first=True)
    ...
    def gen_x_batch(self, q):
        ...
        for i, one_q in enumerate(q):
            if not is_list:
                q_val = list(
                    map(lambda x: self.word_emb.get(x, np.zeros(self.embed_size, dtype=np.float32)), one_q))
            else:
                q_val = []
                for ws in one_q:
                    emb_list = []
    ...
    def forward(self, batch):
        ...
        return [ {
            ...
            "tab_encoding": item3,
            "last_cell": item4,
        } for item1, item2, item3, item4 in zip(src_encoding_list, col_encoding_list, tab_encoding_list, last_cell_lsit)]
