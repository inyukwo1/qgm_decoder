...
from ours.src.rule.graph import Graph
from ours.src.rule.semQL import Sup, Sel, Order, Root, Filter, A, N, C, T, Root1
from ours.src.rule.sem_utils import alter_inter, alter_column0, load_dataSets
...
def _transform(components, transformed_sql, col_set, table_names, schema):
    ...
    while len(components) > 0:
        ...
        if isinstance(c_instance, Root):
            ...
            if table:
                fix_col_id = replace_col_with_original_col(
                    col_set[column.id_c], table_names[table.id_c], current_table
                )
            else:
                ...
                if len(c_instance.production.split()) == 3:
                    ...
                    transformed_sql["where"].append(
                        (
                            ...
                            table_names[table.id_c] if table is not None else table,
                            _transform(
                                components, new_dict, col_set, table_names, schema
                            ),
                        )
                    )
...
def transform(query, schema, idxx=None, origin=None):
    ...
    current_table["schema_content_clean"] = [
        x[1] for x in current_table["column_names"]
    ]
    current_table["schema_content"] = [
        x[1] for x in current_table["column_names_original"]
    ]
    ...
    if c_instance.id_c == 0:
        ...
        _transform(
            components, transformed_sql["intersect"], col_set, table_names, schema
        )
    elif c_instance.id_c == 1:
...
def infer_from_clause(table_names, schema, columns, idxx=None):
    ...
    if len(join_clause) >= 3:
        ...
        if star_table is not None:
            ...
            if star_table_count == 0 and (
                (end_table is None or end_table == star_table)
                or (start_table is None or start_table == star_table)
            ):
    ...
    def reorder_join_tables(join_clause):
        ...
        for i in range(len(join_clause)):
            for j in range(len(join_clause)):
                path = list(
                    schema["graph"].dijkstra(join_clause[i][0], join_clause[j][0])
                )
    ...
    for idx in range(len(join_clause) - 1):
        ...
        if not found:
            ...
            return "FROM " + " JOIN ".join(
                ["%s AS %s" % (jc[0], jc[1]) for jc in join_clause]
            )
    ...
    if len(join_clause) == 1:
        ...
        for idx in range(1, len(join_clause)):
            ...
            clause += " JOIN {} AS {} ON {}.{} = {}.{}".format(
                cur_js[0], cur_js[1], pre_js[1], pre_js[3], cur_js[1], cur_js[2]
            )
        join_clause = clause
...
def replace_col_with_original_col(query, col, current_table):
    ...
    for col_ind, col_name in enumerate(current_table["schema_content_clean"]):
        if col_name == cur_col:
            ...
            if (
                current_table["table_names"][current_table["col_table"][col_ind]]
                == cur_table
            ):
...
def to_str(sql_json, N_T, schema, idxx, pre_table_names=None):
    ...
    if "sup" in sql_json:
        ...
        sup_clause = (
            "ORDER BY %s %s LIMIT 1" % (subject, direction_map[direction])
        ).strip()
        ...
        for f in sql_json["where"]:
            if isinstance(f, str):
                ...
                if value is None:
                    ...
                    if (
                        op == "in"
                        and len(value["select"]) == 1
                        and value["select"][0][0] == "none"
                        ...
                        if value["select"][0][2] not in table_names:
                            table_names[value["select"][0][2]] = "T" + str(
                                len(table_names) + N_T
                            )
                        filters.append(None)
                        ...
                        filters.append(
                            ...
                            % (
                                ...
                                op,
                                "("
                                + to_str(value, len(table_names) + 1, schema, idxx)
                                + ")",
                            )
                        )
                if len(conjunctions):
    ...
    for agg in ["count(", "avg(", "min(", "max(", "sum("]:
        if (
            (len(sql_json["select"]) > 1 and agg in select_clause_str)
            or agg in sup_clause
    ...
    if has_group_by:
        if len(table_names) == 1:
            ...
            for (agg, col, tab) in sql_json["select"]:
                ...
                if agg == "none":
                    group_by_clause = "GROUP BY " + col_to_str(
                        agg, col, tab, table_names, N_T
                    )
                else:
                    is_agg_flag = True
            ...
            if len(sql_json["select"]) == 1:
                ...
                if b:
                    for pair in current_table["foreign_keys"]:
                        t1 = current_table["table_names"][
                            current_table["col_table"][pair[0]]
                        ]
                        t2 = current_table["table_names"][
                            current_table["col_table"][pair[1]]
                        ]
                ...
                if non_agg_count == 1:
                    group_by_clause = "GROUP BY " + col_to_str(
                        non_agg[0], non_agg[1], non_agg[2], table_names, N_T
                    )
                elif non_agg:
                    find_flag = False
                    fix_flag = False
    ...
    table_names_replace = {}
    for a, b in zip(
        current_table["table_names_original"], current_table["table_names"]
    ):
    ...
    if "intersect" in sql_json:
        sql_json["intersect"]["sql"] = sql_json["sql"]
        intersect_clause = "INTERSECT " + to_str(
            sql_json["intersect"], len(table_names) + 1, schema, table_names
        )
    union_clause = ""
    if "union" in sql_json:
        sql_json["union"]["sql"] = sql_json["sql"]
        union_clause = "UNION " + to_str(
            sql_json["union"], len(table_names) + 1, schema, table_names
        )
    except_clause = ""
    if "except" in sql_json:
        sql_json["except"]["sql"] = sql_json["sql"]
        except_clause = "EXCEPT " + to_str(
            sql_json["except"], len(table_names) + 1, schema, table_names
        )
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument(
        "--input_path", type=str, help="predicted logical form", required=True
    )
