    ...
    for label_id, label_val in enumerate(pred_label):
        if label_val in ["Filter(19)"]:
            for primary in schema["primary_keys"]:
                if int(schema["col_table"][primary]) == int(
                    ...
                    pred_label[label_id + 2] = (
                        ...
                        + str(
                            data["col_set"].index(
                                schema["schema_content_clean"][primary]
                            )
                        )
                        + ")"
                    )
            ...
            for pair in schema["foreign_keys"]:
                if int(schema["col_table"][pair[0]]) == h_table and data[
                    "col_set"
                ].index(schema["schema_content_clean"][pair[1]]) == int(
                    ...
                    pred_label[label_id + 8] = (
                        ...
                        + str(
                            data["col_set"].index(
                                schema["schema_content_clean"][pair[0]]
                            )
                        )
                        + ")"
                    )
                    ...
                    break
                elif int(schema["col_table"][pair[1]]) == h_table and data[
                    "col_set"
                ].index(schema["schema_content_clean"][pair[0]]) == int(
                    pred_label[label_id + 2][2:-1]
                ):
                    pred_label[label_id + 8] = (
                        "C("
                        + str(
                            data["col_set"].index(
                                schema["schema_content_clean"][pair[1]]
                            )
                        )
                        + ")"
                    )
