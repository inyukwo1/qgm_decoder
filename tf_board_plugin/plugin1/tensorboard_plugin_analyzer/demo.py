...
def main(unused_argv):
    ...
    with writer.as_default():
        ...
        tf.summary.write(
            "{}_path".format(dataset_name),
            "/home/hkkang/{}".format(dataset_name),
            step=0,
        )
        examples = [
            {
        ...
        for i, example in enumerate(examples):
            ...
            tf.summary.write(
                "{}_query_type".format(dataset_name), example["query_type"], step=i
            )
            tf.summary.write("{}_db".format(dataset_name), example["db"], step=i)
            tf.summary.write(
                "{}_schema".format(dataset_name), example["schema"], step=i
            )
