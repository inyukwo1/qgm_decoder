...
class QGM_Decoder(nn.Module):
    ...
    def decode(self, b_indices, att, state_vector, prev_box=None, gold_boxes=None):
        ...
        x = self.compute_context_vector(b_indices, None, state_vector) if att else torch.zeros(b_size, 600).cuda()
        ...
        state_vector, losses, pred_front_boxes = self.decode_boxes(b_indices, att, state_vector, prev_box=prev_box, gold_boxes=gold_front_boxes)
        pred_boxes = pred_front_boxes

        next_operators = [item for item in selected_idx if item != 0]
        ...
        if next_b_indices is not None:
            ...
            next_state_vector, rear_losses, pred_rear_boxes = self.decode_boxes(next_b_indices, att, next_state_vector, prev_box=prev_boxes, gold_boxes=gold_rear_boxes)
            ...
            state_vector = [list(torch.unbind(state_vector[0])), list(torch.unbind(state_vector[1]))]
    ...
    def decode_boxes(self, b_indices, att, state_vector, prev_box, gold_boxes):
        ...
        groupBy_indices = torch.tensor(groupBy_indices).cuda() if groupBy_indices else None
        orderBy_indices = [idx for idx in range(b_size) if is_orderBy[idx]]
        orderBy_indices = torch.tensor(orderBy_indices).cuda() if orderBy_indices else None

        # Get gold boxes
        groupBy_gold_box = get_box_with_op_type('groupBy', gold_boxes) if self.training else None
        orderBy_gold_box = get_box_with_op_type('orderBy', gold_boxes) if self.training else None
        ...
        if groupBy_indices is not None:
            ...
            groupBy_prev_box = [select_box[idx] for idx in range(b_size) if is_groupBy[idx]]
            groupBy_state_vector = [state_vector[0][groupBy_indices], state_vector[1][groupBy_indices]]

            groupBy_b_indices = torch.tensor([b_indices[idx] for idx in groupBy_indices]).cuda()
            x = self.compute_lstm_input(groupBy_b_indices, groupBy_prev_box, groupBy_att)
            groupBy_state_vector, groupBy_att = self.compute_context_vector(groupBy_b_indices, x, groupBy_state_vector)

            groupBy_loss, groupBy_pred_box = self.decode_box_type('groupBy', groupBy_b_indices, groupBy_att, groupBy_gold_box)
        ...
        if orderBy_indices is not None:
            ...
            orderBy_prev_box = [pred_boxes[idx][-1] for idx in range(b_size) if is_orderBy[idx]]
            orderBy_state_vector = [state_vector[0][orderBy_indices], state_vector[1][orderBy_indices]]

            orderBy_b_indices = torch.tensor([b_indices[idx] for idx in orderBy_indices]).cuda()
            x = self.compute_lstm_input(orderBy_b_indices, orderBy_prev_box, orderBy_att)
            orderBy_state_vector, orderBy_att = self.compute_context_vector(orderBy_b_indices, x, orderBy_state_vector)

            orderBy_loss, orderBy_pred_box = self.decode_box_type('orderBy', orderBy_b_indices, orderBy_att, orderBy_gold_box)
    ...
    def decode_box_type(self, box_type, b_indices, att, gold_box):
        ...
        # Create Boxes
        pred_boxes = [{'head': [], 'body': {'local_predicates': [], 'quantifiers': [], 'quantifier_types': []}, 'operator': BOX_OPS.index(box_type)} for _ in range(b_size)]

        # Predict quantifier type f num
        qf_num_score = layers['qf_num'](att)

        # Apply mask to qf_num_scores
        tab_lens = [list(mask).count(0) for mask in self.tab_mask[b_indices]]
        max_len = qf_num_score.shape[1]
        tab_mask = torch.arange(max_len).expand(b_size, max_len) >= torch.tensor(tab_lens).unsqueeze(1)
        qf_num_score = qf_num_score.masked_fill(tab_mask.cuda(), float('-inf'))
        ...
        # Predict from table
        qf_prob = self.get_attention_weights(self.encoded_tab[b_indices], att, self.tab_mask[b_indices])
        if self.training:
            selected_qf_idx = get_quantifier_with_type('f', gold_box)
            selected_qf_prob = []
            for b_idx in range(b_size):
                tmp_qf_probs = torch.gather(qf_prob[b_idx], 0, torch.tensor(selected_qf_idx[b_idx]).cuda())
            ...
            selected_qf_idx = []
            for b_idx in range(b_size):
                tmp_qf_prob, tmp_qf_idx = torch.topk(qf_prob[b_idx], int(selected_qf_num_idx[b_idx])+1)
        ...
        if box_type != 'order':
            ...
            # Create column mask
            col_masks = torch.ones(self.col_mask[b_indices].shape, dtype=torch.uint8).cuda()
            for idx in range(b_size):
                tmp = []
                for item in selected_qf_idx[idx]:
                    tmp += self.col_tab_dic[b_indices[idx]][item]
                for item in list(set(tmp)):
                    col_masks[idx][item] = 0
            ...
            for n_idx in range(max(selected_p_num_idx)):
                ...
                # Predict Col
                p_col_prob = self.get_attention_weights(self.encoded_col[next_b_indices], p_ctx_vector, col_masks[ori_indices])
            ...
            for idx in range(b_size):
                for sub_idx in range(selected_p_num_idx[idx]):
                    p = (selected_p_agg_indices[idx][sub_idx], selected_p_col_indices[idx][sub_idx], selected_p_op_indices[idx][sub_idx], '')
        ...
        # Predict Head column
        column_prob = self.get_attention_weights(self.encoded_col[b_indices], att, col_masks)
        ...
        if self.training:
            ...
            for b_idx in range(b_size):
                tmp_col_prob, tmp_col_idx = torch.topk(column_prob[b_idx], int(selected_head_num_idx[b_idx])+1)
        ...
        for n_idx in range(max(selected_head_num_idx)+1):
            ...
            # Create context
            tmp_col_idx = torch.tensor([selected_head_col_idx[ori_indices[idx]][n_idx] for idx in range(len(ori_indices))]).cuda()
            col_emb = torch.stack([self.encoded_col[ori_idx][tmp_col_idx[idx]] for idx, ori_idx in enumerate(ori_indices)])
            ...
            if self.training:
                selected_head_agg_idx = to_tensor(get_head_agg_with_idx(n_idx, [gold_box[idx] for idx in ori_indices]))
        ...
        for idx in range(b_size):
            for sub_idx in range(selected_head_num_idx[idx]+1):
                pred_boxes[idx]['head'] += [(selected_head_agg_indices[idx][sub_idx], selected_head_col_idx[idx][sub_idx])]
        ...
        if box_type == 'order':
            ...
            for idx in range(b_size):
                pred_boxes[idx]['is_auxiliary'] = selected_is_asc_idx[idx] == 1
                pred_boxes[idx]['limit_num'] = selected_limit_num_idx[idx]

        # Add loss
        losses = []
        for idx in range(b_size):
            loss = {'head_num': selected_head_num_prob[idx],
                    ...
                    'is_auxiliary': selected_is_asc_prob[idx] if box_type == 'order' else torch.tensor(0),
                    'limit_num': selected_limit_num_prob[idx] if box_type == 'order' else torch.tensor(0)}
            losses += [loss]
    ...
    def encode_box(self, b_indices, boxes):
        ...
        for b_idx, box in enumerate(boxes):
            ...
            for head in box['head']:
                agg_id, col_id = head
                agg_emb = self.agg_emb(agg_id).squeeze(0)
                col_emb = encoded_col[b_idx][col_id]
                emb = agg_emb + col_emb
                tmp += [emb]
    ...
    def attention(self, encoded_src, encoded_src_attn, state_vector, src_mask=None):
        ...
        att_weight = self.get_attention_weights(encoded_src_attn, state_vector, src_mask)
    ...
    def compute_lstm_input(self, b_indices, box, att):
        # Encode Box
        encoded_box = self.encode_box(b_indices, box) if box else torch.zeros(len(att), 300).cuda()
