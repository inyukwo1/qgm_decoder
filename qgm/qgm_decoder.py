...
class QGM_Decoder(nn.Module):
    ...
    def decode(self, b_indices, att, state_vector, prev_box=None, gold_boxes=None):
        ...
        if self.training:
            ...
            op_selected_idx = to_tensor([IUEN.index(op_key) if op_key
        ...
        next_b_indices = [b_indices[idx] for idx in range(b_size) if op_selected_idx[idx] != 0]
    ...
    def decode_box_type(self, box_type, b_indices, att, gold_box):
        ...
        qf_num_mask = torch.arange(max_len).expand(b_size, max_len) >= torch.tensor(tab_lens).unsqueeze(1)
        ...
        # Predict from table
        qf_prob = self.get_attention_weights(self.encoded_tab[b_indices], att, tab_masks, is_log_softmax=True)
        if self.training:
        ...
        if box_type != 'order':
            ...
            for n_idx in range(max(selected_p_num_idx)):
                ...
                # Predict Col
                p_col_prob = self.get_attention_weights(self.encoded_col[next_b_indices], p_ctx_vector, col_masks[ori_indices], is_log_softmax=True)
            ...
            for idx in range(b_size):
                for sub_idx in range(selected_p_num_idx[idx]):
                    p = (int(selected_p_agg_indices[idx][sub_idx]), int(selected_p_col_indices[idx][sub_idx]), int(selected_p_op_indices[idx][sub_idx]), '')
        ...
        for idx in range(b_size):
            for sub_idx in range(selected_head_num_idx[idx]+1):
                pred_boxes[idx]['head'] += [(int(selected_head_agg_indices[idx][sub_idx]), int(selected_head_col_idx[idx][sub_idx]))]
    ...
    def get_attention_weights(self, source, query, source_mask=None, is_log_softmax=False):
        # Compute weight
