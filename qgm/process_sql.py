...
def get_tables_with_alias(schema, toks):
    ...
    for idx, tok in enumerate(toks):
        if tok in SQL_OPS or (
            tok == "(" and idx < len(toks) and toks[idx + 1] == "select"
        ):
...
def parse_condition(toks, start_idx, tables_with_alias, schema, default_tables=None):
    ...
    while idx < len_:
        ...
        if idx < len_ and (
            toks[idx] in CLAUSE_KEYWORDS or toks[idx] in (")", ";", "join")
        ):
            break
    ...
    while idx < len_:
        ...
        if toks[idx] == "select":
            ...
            idx, table_unit, table_name = parse_table_unit(
                toks, idx, tables_with_alias[0], schema
            )
...
def parse_sql(toks, start_idx, tables_with_alias, schema):
    def count_keywords(toks, start_idx, end_idx):
        ...
        for i in range(start_idx, end_idx):
            tok = toks[i]
            if tok in SQL_OPS or (
                tok == "(" and i + 1 < end_idx and toks[i + 1] == "select"
            ):
                cnt += 1
        return cnt
    ...
    from_end_idx, table_units, conds, default_tables = parse_from(
        toks, start_idx, tables_with_alias[table_alias_idx:], schema
    )
    sql["from"] = {"table_units": table_units, "conds": conds}
    # select clause
    _, select_col_units = parse_select(
        toks, start_idx, tables_with_alias[table_alias_idx:], schema, default_tables
    )
    ...
    # where clause
    where_end_idx, where_conds = parse_where(
        toks, idx, tables_with_alias[table_alias_idx:], schema, default_tables
    )
    ...
    # group by clause
    group_end_idx, group_col_units = parse_group_by(
        toks, idx, tables_with_alias[table_alias_idx:], schema, default_tables
    )
    ...
    # order by clause
    order_end_idx, order_col_units = parse_order_by(
        toks, idx, tables_with_alias[table_alias_idx:], schema, default_tables
    )
    ...
    # having clause
    having_end_idx, having_conds = parse_having(
        toks, idx, tables_with_alias[table_alias_idx:], schema, default_tables
    )
    ...
    if idx < len_ and toks[idx] in SQL_OPS:
        sql_op = toks[idx]
        idx += 1
        idx, IUE_sql = parse_sql(
            toks, idx, tables_with_alias[table_alias_idx + 1 :], schema
        )
        sql[sql_op] = IUE_sql
    return idx, sql
