...
def random_choice(
    question_arg, question_arg_type, names, ground_col_labels, q_ind, N, origin_name
):
        ...
        if i < len(question_arg):
            if len(ground_col_labels) == 0:
                ...
                for n_id, n in enumerate(names):
                    if (
                        n_id in ground_col_labels
                        and partial_match(question_arg[i][0], n) is True
                    ):
                        return n
    if len(ground_col_labels) > 0:
    ...
    for i in range(len(question_arg_type))[::-1]:
        if question_arg_type[i] == ["NONE"]:
            for t_id, table_name in enumerate(origin_table_names):
                if (
                    partial_match(question_arg[i], table_name) is True
                    and t_id != h_table
                ):
                    return t_id
...
def alter_not_in_one_entry(d, current_table):
    if "Filter(19)" in d["model_result"]:
        ...
        origin_table_names = [
            [wordnet_lemmatizer.lemmatize(x.lower()) for x in names.split(" ")]
        ...
        h_table = find_table(
            cur_table, origin_table_names, question_arg_type, question_arg
        )

        for label_id, label_val in enumerate(pred_label):
            if label_val in ["Filter(19)"]:
                for primary in current_table["primary_keys"]:
                    if int(current_table["col_table"][primary]) == int(
                        pred_label[label_id - 1][2:-1]
                    ):
                        pred_label[label_id + 2] = (
                            "C("
                            + str(
                                d["col_set"].index(
                                    current_table["schema_content_clean"][primary]
                                )
                            )
                            + ")"
                        )
                ...
                for pair in current_table["foreign_keys"]:
                    if int(current_table["col_table"][pair[0]]) == h_table and d[
                        "col_set"
                    ].index(current_table["schema_content_clean"][pair[1]]) == int(
                        pred_label[label_id + 2][2:-1]
                    ):
                        pred_label[label_id + 8] = (
                            "C("
                            + str(
                                d["col_set"].index(
                                    current_table["schema_content_clean"][pair[0]]
                                )
                            )
                            + ")"
                        )
                        ...
                        break
                    elif int(current_table["col_table"][pair[1]]) == h_table and d[
                        "col_set"
                    ].index(current_table["schema_content_clean"][pair[0]]) == int(
                        pred_label[label_id + 2][2:-1]
                    ):
                        pred_label[label_id + 8] = (
                            "C("
                            + str(
                                d["col_set"].index(
                                    current_table["schema_content_clean"][pair[1]]
                                )
                            )
                            + ")"
                        )
...
def alter_inter_one_entry(d):
    if "Filter(0)" in d["model_result"]:
        ...
        if c1 != c2 or c1 is None or c2 is None:
            ...
            replace_result = ["Root1(0)"] + now_result[
                1 : now_result.index("Filter(0)")
            ]
            for r_id, r_val in enumerate(
                now_result[now_result.index("Filter(0)") + 2 :]
            ):
                if "Filter" in r_val:
                    break
            ...
            replace_result = (
                replace_result + now_result[1 : now_result.index("Filter(0)")]
            )

            replace_result = (
                replace_result + now_result[r_id + now_result.index("Filter(0)") + 2 :]
            )
...
def alter_column0(datas):
    ...
    for re in result:
        table_names = [
            [lemma(x) for x in names.split(" ")] for names in re[3]["table_names"]
        ]
        origin_table_names = [
            [x for x in names.split(" ")] for names in re[3]["table_names"]
        ]
    ...
    for data in datas:
        if "rule_count" in data:
            ...
            replace_result = regex.sub(
                "C\(0\) T\(.\)", str_replace, data["model_result"]
            )
...
def alter_column0_one_entry(d):
    ...
    if "C(0)" in d["model_result"]:
        ...
        question_arg = d["question_arg"]
        table_names = [
            [lemma(x) for x in names.split(" ")] for names in d["table_names"]
        ]
        origin_table_names = [
            [wordnet_lemmatizer.lemmatize(x.lower()) for x in names.split(" ")]
            for names in d["table_names"]
        ...
        if easy_flag:
            ...
            for q_ind, q in enumerate(d["question_arg"]):
                if (
                    (
                        q_ind > 0
                        and q == ["many"]
                        and d["question_arg"][q_ind - 1] == ["how"]
                    )
                    or (
                        q_ind > 0
                        and q == ["of"]
                    ...
                    or (
                        q_ind > 0
                        and q == ["of"]
                        and d["question_arg"][q_ind - 1] == ["count"]
                    )
                ):
            ...
            for q_ind, q in enumerate(d["question_arg"]):
                if (
                    M_OP is False
                    and q in [["than"], ["least"], ["most"], ["msot"], ["fewest"]]
                    or question_arg_type[q_ind] == ["M_OP"]
                ):
                    M_OP = True
