...
class IRNet(BasicModel):
    ...
    def forward(self, examples, transformer_encoder):
        args = self.args
        ...
        if args.bert == -1:
            ...
            (
                ...
                schema_embedding,
                last_cell,
            ) = self.transformer_encode(batch, transformer_encoder)
    ...
    def transformer_encode(self, batch: Batch, transformer_encoder):
        B = len(batch)
        ...
        embedding = transformer_encoder(encoded_questions)[0]
        src_encodings = []
        table_embedding = []
        schema_embedding = []
    ...
    def parse(self, examples, transformer_encoder, beam_size=5):
        """
        one example a time
        :param examples:
        :param beam_size:
