...
def _prepare_model(path, config_id, model_id, voc_id_1, voc_id_2, voc_id_3):
    ...
    download_file_from_google_drive(
        voc_id_1, path + "/vocabulary/non_padded_namespaces.txt"
    )
...
def prepare_model(dataset):
    ...
    if dataset == "geo":
        _prepare_model(
            model_path,
            "16qJ73v5TzaKmj-8csAfbZEqA7FvnpAiP",
        ...
        _prepare_model(
            model_path,
            "1cqk3X6_YGmFMwLf6Ueoca7QM2v67qIER",
        ...
        _prepare_model(
            model_path,
            "1euVD-pkKMOK_NAJhrmBQPDukgKbnmiZn",
        ...
        _prepare_model(
            model_path,
            "1DJPg7Zx5FQlvYqagbH72o14H-mlk6b1v",
        ...
        _prepare_model(
            model_path,
            "1ZpPLIc6jQuuLouFiKabdYJCgIr0uPH_n",
        ...
        _prepare_model(
            model_path,
            "1mlr7hnl6qDnaplcZEw7o06cGrmr6DRjW",
        ...
        _prepare_model(
            model_path,
            "1w1vj6t-4arb4I7D5cDdoppDVR4vQANSI",
        ...
        _prepare_model(
            model_path,
            "1uGE0M8oRaeRrDw5ZZlR0yFuiZxz0d-J5",
        ...
        _prepare_model(
            model_path,
            "10lNyTPMGd3z5O_U9d7oWxGih1Diq_93e",
...
def test_end2end_geo():
    ...
    assert (
        q
        == """select city.city_name from city where city.state_name = ' value ' and city.population = ( select max ( city.population ) from city where city.state_name = ' value ' )"""
    )
...
def test_end2end_mas():
    ...
    q, _, _ = end2end_gnn.run_model(
        "mas",
        'return me the number of researchers in database area in "University of Michigan"',
    )
...
def test_end2end_patients():
    ...
    q, _, _ = end2end_gnn.run_model(
        "patients", "aggregate the ages of all patients in the database"
    )
...
def test_end2end_restaurant():
    ...
    q, _, _ = end2end_gnn.run_model(
        "restaurant", "give me a restaurant in the bay area ?"
    )
    assert q == "NO PREDICTION"
...
def test_end2end_scholar():
    ...
    q, _, _ = end2end_gnn.run_model(
        "scholar", "What papers has Sharon Goldwater written ?"
    )
    assert (
        q
        == """select distinct paper.venueid from writes , author where writes.authorid = author.authorname and author.authorname = ' value '"""
    )
...
def test_end2end_tpch():
    ...
    q, _, _ = end2end_gnn.run_model(
        "tpch",
        "Find a list of the top 100 customers who have ever placed large quantity orders. The query lists the customer name, customer key, the order key, date and total priceand the quantity for the order",
    )
    assert (
        q
        == """select orders.order_key , orders.total_price from orders order by orders.total_price desc limit 1"""
    )
...
def test_end2end_yelp():
    ...
    q, _, _ = end2end_gnn.run_model(
        "yelp",
        "Find the number of reviews on businesses located in South Summerlin neighborhood",
    )
