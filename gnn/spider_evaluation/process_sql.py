...
def tokenize(string, has_value=True):
    ...
    assert len(quote_idxs) % 2 == 0, "Unexpected quote "+string+", "+' '.join(str(x) for x in quote_idxs)
    ...
    for i in range(len(toks_no_value)):
        ...
        elif bool(FLOAT_MATCH(toks_no_value[i])) and i != 0 and toks_no_value[i-1] in WHERE_OPS: 
            toks_no_value[i] = "value"
...
def scan_alias(toks, tables):
    ...
    for idx in table_idxs:
        if idx < len(toks)-1 and toks[idx+1] not in special_tokens and toks[idx+1] not in CLAUSE_KEYWORDS and toks[idx+1] not in WHERE_OPS and toks[idx+1] not in UNIT_OPS and toks[idx+1] not in AGG_OPS and toks[idx+1] not in COND_OPS and toks[idx+1] not in ORDER_OPS:
...
def parse_condition(toks, start_idx, tables_with_alias, schema, default_tables=None):
    ...
    while idx < len_:
        ...
        if toks[idx] in COND_OPS:
            ...
            if len(not_points) % 2 == 1: # not (A and B) -> (not A or not B), not (A or B) -> (not A and not B)
               if cond_op_id == 0:
                  cond_op_id = 1
               elif cond_op_id == 1:
                  cond_op_id = 0
        ...
        elif op_id == WHERE_OPS.index('in') and idx < len_-2 and ( '"' in toks[idx+1] or is_number_tryexcept(toks[idx+1]) or 'value' in toks[idx+1] ):
            assert toks[idx] == '(' 
            idx +=1 
            idx, val1 = parse_value(toks, idx, tables_with_alias, schema, default_tables)
            if toks[idx] == ')':
               ...
               assert toks[idx] == ')' 
               idx += 1
        elif toks[idx][0] not in STR_VALUES_START_WITH and is_number_tryexcept(toks[idx]) is False: # (COL) (OP) (COL)
            idx, val1 = parse_val_unit(toks, idx, tables_with_alias, schema, default_tables)
            val2 = None
    ...
    bracket_points = [tuple([-1, len(conds)]),]
    cur_bracket_stack = []
    ...
    or_bracket_ids = [max(find_brackets(or_point, bracket_points)) for or_point in or_points]
    join_conds = []
    conds_res = []
    ...
    for cond_idx in range(len(conds)):
        ...
        for or_bracket_id in or_bracket_ids:
             if or_bracket_id in bracket_ids:
                 has_or = True
                 break
        if isinstance(cond, tuple) and isinstance(cond[3], tuple) and not has_or: # join cond
            ...
            if isinstance(cond, tuple):
               ...
               if not (isinstance(conds_res[-1], tuple) or conds_res[-1] == ')'):
                  conds_res.pop()
               if len(conds_res) > 0 and ( isinstance(conds_res[-1], tuple) or conds_res[-1] == ')'):
                  conds_res.append(cond)
...
def parse_select(toks, start_idx, tables_with_alias, schema, default_tables=None):
    ...
    while idx < len_ and toks[idx] not in CLAUSE_KEYWORDS:
        ...
        if idx < len_ and toks[idx] in UNIT_OPS:
            agg_id = AGG_OPS.index("none")
            idx, val_unit = parse_val_unit(toks, pre_idx, tables_with_alias, schema, default_tables)
...
def parse_from(toks, start_idx, tables_with_alias, schema):
    ...
    while idx < len_:
        ...
        if toks[idx] == 'select':
            ...
            idx += 1  # skip on
            idx, _, this_conds = parse_condition(toks, idx, tables_with_alias, schema, default_tables)
...
def parse_where(toks, start_idx, tables_with_alias, schema, default_tables):
    ...
    idx += 1
    idx, conds, join_conds = parse_condition(toks, idx, tables_with_alias, schema, default_tables)
    return idx, conds, join_conds
...
def parse_having(toks, start_idx, tables_with_alias, schema, default_tables):
    ...
    idx += 1
    idx, conds, _ = parse_condition(toks, idx, tables_with_alias, schema, default_tables)
    return idx, conds
...
def parse_sql(toks, start_idx, tables_with_alias, schema, mapped_entities_fn=None):
    ...
    # where clause
    idx, where_conds, join_conds = parse_where(toks, idx, tables_with_alias, schema, default_tables)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--parenthesize', action='store_true', default=False, dest='parenthesize_switch') 
...
#        continue
        ...
        try:
           ...
           toks = tokenize(query)
           toks_as, tables_with_alias = get_tables_with_alias(schema.schema, toks, schema_origin.keys())
    #       print toks
