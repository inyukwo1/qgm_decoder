...
def scan_alias(toks, tables):
    ...
    for idx in table_idxs:
        if idx < len(toks) and toks[idx+1] not in special_tokens and toks[idx+1] not in CLAUSE_KEYWORDS:
...
def parse_condition(toks, start_idx, tables_with_alias, schema, default_tables=None):
    ...
    while idx < len_:
        ...
        elif op_id == WHERE_OPS.index('in') and idx < len_-2 and ( '"' in toks[idx+1] or is_number_tryexcept(toks[idx+1]) ):
            assert toks[idx] == '(' 
            idx +=1 
            idx, val1 = parse_value(toks, idx, tables_with_alias, schema, default_tables)
            if toks[idx] == ')':
