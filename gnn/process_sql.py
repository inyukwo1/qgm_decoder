...
def parse_condition(toks, start_idx, tables_with_alias, schema, default_tables=None):
    ...
    while idx < len_:
        ...
        if op_id == WHERE_OPS.index(
            ...
            and idx < len_ - 2
            and ('"' in toks[idx + 1] or is_number_tryexcept(toks[idx + 1]))
        ):
            assert toks[idx] == "("
            idx += 1
            idx, val1 = parse_value(
    ...
    print(
        "INITIAL_CONDS: {}, bracket_points: {}, or_points: {}, or_bracket_ids: {}".format(
            conds_print, bracket_points, or_points, or_bracket_ids
        )
    )
