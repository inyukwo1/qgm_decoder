...
def condition_has_or(conds):
    cond_ao_unit = [
        cond_unit for cond_unit in conds if not isinstance(cond_unit, (list, tuple))
    ]
...
def condition_has_like(conds):
    cond_list_unit = [
        ...
        if isinstance(cond_unit, (list, tuple)) and len(cond_unit) > 1
    ]
...
def eval_where(pred, label):
    pred_conds = [
        unit for unit in pred["where"] if isinstance(unit, list) and len(unit) > 1
    ]
    label_conds = [
        unit for unit in label["where"] if isinstance(unit, list) and len(unit) > 1
    ]
...
def eval_and_or(pred, label):
    ...
    pred_ao_unit = [
        cond_unit
        for cond_unit in pred["where"]
        if not isinstance(cond_unit, (list, tuple))
    ]
    pred_ao = pred_ao_unit
    label_ao_unit = [
        cond_unit
        for cond_unit in label["where"]
        if not isinstance(cond_unit, (list, tuple))
    ]
...
def get_nestedSQL(sql):
    ...
    from_cond_list_unit = [
        cond_unit
        for cond_unit in sql["from"]["conds"]
        if isinstance(cond_unit, (list, tuple)) and len(cond_unit) > 1
    ]
    where_list_unit = [
        cond_unit
        for cond_unit in sql["where"]
        if isinstance(cond_unit, (list, tuple)) and len(cond_unit) > 1
    ]
    having_list_unit = [
        cond_unit
        for cond_unit in sql["having"]
        if isinstance(cond_unit, (list, tuple)) and len(cond_unit) > 1
    ]
