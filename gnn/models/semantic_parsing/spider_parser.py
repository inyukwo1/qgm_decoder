...
from gnn.state_machines.transition_functions.linking_transition_function import LinkingTransitionFunction
...
class MaximumMarginalLikelihood2(MaximumMarginalLikelihood):
    ...
    def decode(self,
        ...
        not_finished_state_indices=[i for i in range(len(targets)) if i not in finished_states]
        loss = 0
        ...
        if len(finished_states) == 0:
            return {'loss': torch.tensor(0.0, requires_grad=True) * torch.tensor(10), 'not_finished_state_indices': not_finished_state_indices}
        else:
            return {'loss': loss / len(finished_states), 'not_finished_state_indices': not_finished_state_indices}
        ...
        if self.training:
            ...
            if 'not_finished_state_indices' in decode_output:
                for index in decode_output['not_finished_state_indices']:
                    print("TRAINING ERROR -- index: {}, question: {}, query: {}".format(query_index[index], utterance_string[index], ' '.join(world[index].query)))
                ...
                try:
                    ...
                    if 'not_finished_state_indices' in decode_output:
                        for index in decode_output['not_finished_state_indices']:
                            ...
                            print("DEV/TEST ERROR -- index: {}, question: {}, query: {}".format(query_index[index], utterance_string[index], ' '.join(world[index].query)))
