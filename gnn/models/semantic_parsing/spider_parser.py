...
from allennlp.state_machines.states import GrammarStatelet, State
...
from gnn.modules.gated_graph_conv import GatedGraphConv
from gnn.semparse.worlds.evaluate_spider import evaluate
from gnn.state_machines.states.rnn_statelet import RnnStatelet
from allennlp.state_machines.trainers import MaximumMarginalLikelihood
from allennlp.training.metrics import Average
from overrides import overrides

from gnn.semparse.contexts.spider_context_utils import action_sequence_to_sql
from gnn.semparse.worlds.spider_world import SpiderWorld
from gnn.state_machines.states.grammar_based_state import GrammarBasedState
from gnn.state_machines.states.sql_state import SqlState
from gnn.state_machines.transition_functions.attend_past_schema_items_transition import (
    AttendPastSchemaItemsTransitionFunction,
)
from gnn.state_machines.transition_functions.linking_transition_function import (
    LinkingTransitionFunction,
)


training_invalid_queries = set()
dev_test_invalid_queries = set()


class MaximumMarginalLikelihood2(MaximumMarginalLikelihood):
    ...
    def decode(
        ...
        not_finished_state_indices = [
            i for i in range(len(targets)) if i not in finished_states
        ]
        loss = 0
        ...
        if len(finished_states) == 0:
            return {
                "loss": torch.tensor(0.0, requires_grad=True) * torch.tensor(10),
                "not_finished_state_indices": not_finished_state_indices,
            }
        else:
            return {
                "loss": loss / len(finished_states),
                "not_finished_state_indices": not_finished_state_indices,
            }
...
class SpiderParser(Model):
    def __init__(
        ...
        self._decoder_trainer = MaximumMarginalLikelihood2(training_beam_size)
    ...
    def forward(
        ...
        action_sequence_str: List[str],
        action_sequence: torch.LongTensor = None,
    ) -> Dict[str, torch.Tensor]:
        global training_invalid_queries
        global dev_test_invalid_queries
        ...
        if self.training:
            ...
            if "not_finished_state_indices" in decode_output:
                for index in decode_output["not_finished_state_indices"]:
                    print(
                        "TRAINING ERROR -- index: {}, question: {}, query: {}".format(
                            query_index[index],
                            utterance_string[index],
                            " ".join(world[index].query),
                        )
                    )
                    training_invalid_queries.add(query_index[index])
            ...
            if action_sequence is not None and action_sequence.size(1) > 1:
                try:
                    ...
                    if "not_finished_state_indices" in decode_output:
                        for index in decode_output["not_finished_state_indices"]:
                            dev_test_invalid_queries.add(query_index[index])
                            print(
                                "DEV/TEST ERROR -- index: {}, question: {}, query: {}".format(
                                    query_index[index],
                                    utterance_string[index],
                                    " ".join(world[index].query),
                                )
    ...
    def _get_initial_state(
        ...
        if self._gnn:
            ...
            encoder_output_dim = (
                self._action_embedding_dim + self._encoder.get_output_dim()
            )
        else:
        ...
        initial_sql_state = [
            SqlState(actions[i], self.parse_sql_on_decoding) for i in range(batch_size)
        ]
    ...
    def is_nonterminal(token: str):
        if token[0] == '"' and token[-1] == '"':
            return False
        if token[0] == "'" and token[-1] == "'":  # [HKKANG] for WikiSQL
            return False
        return True
    ...
    def _action_history_match(predicted: List[int], targets: torch.LongTensor) -> int:
        ...
        return torch.max(
            torch.min(targets_trimmed.eq(predicted_tensor), dim=0)[0]
        ).item()
    ...
    def _query_difficulty(targets: torch.LongTensor, action_mapping, batch_index):
        number_tables = len(
            [
                ...
                for a in targets
                if a >= 0
                and action_mapping[(batch_index, int(a))].startswith("table_name")
            ]
        )
