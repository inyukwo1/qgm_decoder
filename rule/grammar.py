...
class Grammar(nn.Module):
    ...
    # Get next action
    def get_possible_aids(self, symbol_or_symbol_id):
        if isinstance(symbol_or_symbol_id, str):
            symbol = symbol_or_symbol_id
        else:
            symbol = self.sid_to_symbol[symbol_or_symbol_id]
        return [
