...
def get_col_set_idx(column_names, col_id):
    col_name_set = []
    for col_name in column_names:
        if col_name not in col_name_set:
            col_name_set += [col_name]
...
def create_group_by_box(sql, info, schema):
    ...
    if info["groupBy"]:
        ...
        for idx in range(0, len(info["having"]), 2):
            ...
            if col_id != 0:
                tab_id = schema["column_names"][col_id][0]
                quantifiers += [tab_id]
                quantifier_types += ["f"]
            else:
                tab_id = 0
            ...
            local_predicates += [(agg_id, (col_id, tab_id), operator, right_operand)]
        body = {
...
def create_select_box(sql, info, schema):
    ...
    for idx in range(0, len(info["where"]), 2):
        ...
        column_names = [item[1] for item in schema["column_names"]]
        col_set_id = get_col_set_idx(column_names, col_id)
        local_predicates += [(agg_id, (col_set_id, tab_id), operator, right_operand)]
