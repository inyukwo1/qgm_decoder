...
class TransformerBatchState:
    def __init__(self,
        ...
        self.step_cnt = 0
        # Global
        self.nonterminal_stack = [[grammar.get_start_symbol_id()] for _ in range(self.get_b_size())]
    ...
    def _combine_view(self, view):
        if view:
            ...
            for idx, b_idx in enumerate(view.b_indices):
                assert len(self.pred_history[b_idx]) + 1 == len(view.pred_history[b_idx])
    ...
    def split_by_view(self, out):
        # Get view indices
        action_view_indices, column_view_indices, table_view_indices = self._get_view_indices()

        return out[action_view_indices], out[column_view_indices], out[table_view_indices]
    ...
    def update_state(self):
        ...
        if self.step_cnt == 30:
            ...
            next_indices = [idx for idx, b_idx in enumerate(self.b_indices) if self.nonterminal_stack[b_idx]]
    ...
    def get_tgt(self):
        ...
        # Get prev symbols
        prev_symbols = [self.pred_history[idx] if self.pred_history[idx] else [] for idx in view_indices]
    ...
    def save_pred(self, preds):
        for idx, b_idx in enumerate(self.b_indices):
            if self.state_type == 'action':
                ...
                next_symbols = [symbol for symbol in self.grammar.get_next_action(*action).split(' ') if symbol not in self.grammar.terminals]
                next_symbol_ids = [self.grammar.symbol_to_symbol_id[symbol] for symbol in next_symbols]
                self.nonterminal_stack[b_idx] = next_symbol_ids + self.nonterminal_stack[b_idx][1:]
