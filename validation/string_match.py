...
import itertools
from validation.canonicaliser_after_alias import *
import signal
import MySQLdb
...
def get_permutation_maps(dic):
    ...
    for table in dic:
        ...
        new_dic[table] = [
            {from_names[i]: n for i, n in enumerate(p)} for p in permutations
        ]
    return new_dic
...
def get_generated_queries(cur, dataset, system, category):
    ...
    WHERE Q.query_id=R.query_id AND Q.query_id=S.query_id
      ...
      AND (S.split="test" OR (S.split="dev" AND S.db="spider"))
      AND S.category="{category}"
      AND E.system="{system}"
      AND REPLACE(E.test_data, '-s', '')=Q.db
    """.format(
        system=system, category=category, dataset=dataset
    )
    cur.execute(sql)
    rows = cur.fetchall()
    gen_queries = {}
    ...
    for idx, gen_sql, nl in rows:
        key = "\t".join([dbname, str(idx), category])
...
def write_file(
    rewrite_query_file,
    info_file,
    read_gold_query_files,
    schema_filename,
    dataset,
    acc_name="accurate_qm_db2",
):
    ...
    with open(rewrite_query_file) as f_q, open(info_file) as f_i:
        ...
        while True:
            ...
            if not query_line or not info_line:
                assert not is_written or (not query_line and not info_line)
                break
    ...
    ### read gold queries ###
    origin_gold_queries = {}
    for read_gold_query_file, category in read_gold_query_files:
        if not isfile(read_gold_query_file):
            continue
        with open(read_gold_query_file) as f:
            ...
            while True:
                ...
                if not line:
                    break
                datum = line.strip(" \n\r").split("\t")
                key = "{}\t{}\t{}".format(datum[0].lower(), datum[1], category)
                origin_gold_queries[key] = datum

    ### Do string match -> write results file ###
    write_result_form = "./results/{sys}_{db}_{db}_{category}.results"
    write_files = []
    with closing(
        MySQLdb.connect(host=HOST, user=USER, passwd=PASSWD, charset="utf8")
    ) as conn, closing(conn.cursor()) as cur:
        cur.execute("use benchmark")
        for sys, category in gen_queries:
            gen_execute_queries, nl_questions = get_generated_queries(
                cur, dataset, sys, category
            )
            ...
            with open(write_result_file, "w") as f_result:
                ...
                for key in keys:
                    ...
                    if sql_cosette == "" or gold_query == "":
                        ...
                        acc_qm = timeout(
                            is_equal_query,
                            args=(sql_cosette, gold_query, schema),
                            timeout_duration=30,
                            default=0,
                        )
                        if acc_qm:
                    ...
                    if acc_qm:
                        ...
                        sql_execute = gen_execute_queries[key]
                        nl = nl_questions[key]
                        # sql_cosette = order_query(sql_cosette, set())
                        # gold_query = order_query(gold_query, set())
                        print(
                            "{}".format(
                                "\t".join(
                                    [
...
def get_gold_filenames(dbname, dataset):
    ...
    files = []
    for cat in ["basic", "canonicalize", "seen"]:
        names["category"] = cat
...
if __name__ == "__main__":
    info_dir = "./cosette_errors/"
    query_dir = "./canonicalized_files/"
    info_form = "./cosette_errors/{system}_{db}_cosette_result_index.tsv"
    query_form = "./canonicalized_files/{system}_{db}_canonicalized.sql"

    info_files = [
        join(info_dir, f) for f in listdir(info_dir) if isfile(join(info_dir, f))
    ]
    ...
    if info_file in info_files or query_file in query_files:
        accuracy_filenames = write_file(
            query_file, info_file, gold_query_files, schema_filename, dataset
        )
    else:
        print("No file(s)")
