...
def compute_ex(dbfile, gen_sql, gold_sql):
    ...
    if len(gold_output) == 0 and len(gen_output) == 0:
        ...
        gold_cols = tuple(
            tuple(pack_column(gold_output, i) for i in range(len(gold_output[0])))
        )
        gen_cols = tuple(
            tuple(pack_column(gen_output, i) for i in range(len(gen_output[0])))
        )
        accurate_ex = True if Counter(gold_cols) == Counter(gen_cols) else False
    else:
        gold_cols = tuple(
            pack_column(gold_output, i) for i in range(len(gold_output[0]))
        )
        ...
        if len(gold_cols) != len(gen_cols) or len(gold_output) != len(gen_output):
            ...
            permus = [
                ...
                for permu in permutations(
                    [i for i in range(len(gold_cols))], len(gold_cols)
                )
            ]
            mappings = list(
                mapping for mapping in product(*row_maps) if mapping in permus
            )
            accurate_ex = False
            for mapping in mappings:
                gen_cols_permus = tuple(gen_cols[i] for i in mapping)
                gen_output_permus = tuple(
                    pack_column(gen_cols_permus, i)
                    for i in range(len(gen_cols_permus[0]))
                )
    ...
    try:
        with open(fname, "w") as f:
            tables = pd.read_sql_query(
                "SELECT name FROM sqlite_master WHERE type='table'", disk_engine
            )
            for table_name in tables.values:
                table_name = table_name[0]
                columns = pd.read_sql_query(
                    "PRAGMA table_info('%s')" % table_name, disk_engine
                )
...
def compute_string_match(dbfile, dbname, gen_sql, gold_sql):
    ...
    try:
        ...
        accurate_str = timeout(
            is_equal_query,
            args=(gen_canonical, gold_canonical, schema),
            timeout_duration=30,
            default=0,
        )
...
if __name__ == "__main__":
    ...
    print(
        diff_two_queries("sqlite:///imdb.sqlite", "imdb", gen_sql, gold_sql, "origin")
    )
    print(
        diff_two_queries(
            "sqlite:///imdb.sqlite", "imdb", gen_sql, gold_sql, "canonical"
        )
    )
