...
from framework.lazy_modules import (
    LazyDotProductAttention,
    LazyLinearTanhDropout,
    LazyLSTMCell,
    LazyTransformerDecoder,
    LazyLinearLinear,
    LazyPointerNet,
    LazyMemoryPointerNet,
...
class SemQLDecoderFramework(nn.Module):
    def __init__(self, cfg):
        ...
        self.col_for_input = nn.Linear(dim, dim, False)
        self.tab_for_input = nn.Linear(dim, dim, False)
        ...
        self.table_pointer_net = LazyPointerNet(dim, dim, with_candidate=True)
        ...
        self.action_linear_layer = LazyLinearLinear(
            dim, len(self.grammar.aid_to_action)
        )

        self.action_similarity = LazyCalculateSimilarity(dim, dim)

        self.sketch_lstm = LazyLSTM(dim * 2, dim)
        self.tgt_linear_layer = LazyLinear(dim * 2, dim)
        self.decoder_transformer = LazyTransformerDecoder(dim, 6, 3)

        self.detail_action_similarity = LazyCalculateSimilarity(dim, dim)
        self.detail_column_similarity = LazyCalculateSimilarity(dim, dim)
        self.detail_table_similarity = LazyCalculateSimilarity(dim, dim)
    ...
    def action_to_emb(self, state: LSTMState, action: Action) -> torch.Tensor:
        if action[0] == "C":
            col_idx = action[1]
            return self.col_for_input(state.encoded_col[col_idx])
        elif action[0] == "T":
            tab_idx = action[1]
            return self.tab_for_input(state.encoded_tab[tab_idx])
        else:
    ...
    def further_encode(self, inputs, module):
        ...
        stacked_input = torch.zeros(
            len(inputs), max(data_len), inputs[0].shape[1]
        ).cuda()
    ...
    def forward(
        ...
        def embed_sketch_action(state: LSTMState, _) -> Dict:
            history_actions: List[Action] = state.get_prev_sketches()
            history_action_embeddings = [
                self.action_to_emb(state, action) for action in history_actions
            ]
            current_action_embedding = self.one_dim_zero_tensor()
            ...
            #     action_embeddings
            # )
            return {"action_emb": action_embeddings}
            # action = state.get_prev_sketch()
            # action_emb = (
            #     self.action_to_emb(state, action)
            #     if action
            #     else self.one_dim_zero_tensor()
            # )
            # prev_tensor_dict = {"action_emb": action_emb}
            # return prev_tensor_dict

        def embed_sketch_symbol(
            state: LSTMState, prev_tensor_dict: Dict[str, Tensor]
        ) -> Dict:
            history_actions: List[Action] = state.get_prev_sketches()
            history_symbols = [item[0] for item in history_actions] + [
                state.get_sketch_current_symbol()
            ]
            symbol_id_list = [
                self.grammar.symbol_to_sid[symbol] for symbol in history_symbols
            ]
            symbol_ids_tensor = torch.tensor(symbol_id_list).long().cuda()
            symbol_embeddings = self.grammar.symbol_emb(symbol_ids_tensor)
            prev_tensor_dict.update({"symbol_emb": symbol_embeddings})
            return prev_tensor_dict

            # action = state.get_prev_sketch()
            # symbol_emb = (
            #     self.grammar.symbol_to_emb(action[0])
            #     if action
            #     else self.one_dim_zero_tensor()
            # )
        ...
        def pass_sketch_lstm_cell(
            state: LSTMState, prev_tensor_dict: Dict[str, Tensor]
        ) -> Dict:
            action_emb = prev_tensor_dict["action_emb"]
            symbol_emb = prev_tensor_dict["symbol_emb"]
            attn_emb = state.get_att_emb()
            combined_input = torch.cat([action_emb, symbol_emb], dim=-1)
            # combined_input = torch.cat([action_emb, symbol_emb], dim=-1)
            lstm_state = state.get_sketch_state()

            tgt_promise = self.tgt_linear_layer.forward_later(combined_input)
            prev_tensor_dict.update({"combined_input": tgt_promise})
            return prev_tensor_dict

        def decode(state, prev_tensor_dict):
            combined_input = prev_tensor_dict["combined_input"].result
            decoder_out_promise = self.decoder_transformer.forward_later(
                combined_input, state.encoded_src
            )

            prev_tensor_dict.update({"decoder_out": decoder_out_promise})
            return prev_tensor_dict

        # def update_sketch_lstm_state(
        #     state: LSTMState, prev_tensor_dict: Dict[str, TensorPromiseOrTensor]
        # ) -> Dict:
        #     # new_lstm_state = prev_tensor_dict["new_lstm_state"].result
        #     # state.update_sketch_state(new_lstm_state)
        #     return prev_tensor_dict
        #
        # def pass_sketch_att_src(
        #     state: LSTMState, prev_tensor_dict: Dict[str, TensorPromiseOrTensor]
        # ) -> Dict:
        #     # h_state, _ = state.get_sketch_state()
        #     h_state = prev_tensor_dict["new_lstm_state"].result
        #     src_encodings = state.get_src_encodings()
        #     aff_sketch_src = state.get_affine_src()
        #     att_out = self.sketch_dot_product_attention.forward_later(
        #         aff_sketch_src, h_state, src_encodings
        #     )  # dot product attention + linear + tanh + dropout
        #     prev_tensor_dict.update({"att_out": att_out})
        #     return prev_tensor_dict
        #
        # def update_sketch_att_src(
        #     state: LSTMState, prev_tensor_dict: Dict[str, TensorPromiseOrTensor]
        # ) -> Dict:
        ...
        def calc_sketch_prob(
            state: LSTMState, prev_tensor_dict: Dict[str, TensorPromiseOrTensor]
        ) -> Dict:
            # Get current symbol
            cur_symbol = state.get_sketch_current_symbol()
            action_ids = self.grammar.get_possible_aids(cur_symbol)
            impossible_indices = [
                idx
                for idx in range(SemQL.semql.get_action_len())
                if idx not in action_ids
            ]
            action_prob = self.action_similarity.forward_later(
                prev_tensor_dict["decoder_out"].result[-1],
            ...
            # action_prob = self.action_linear_layer.forward_later(
            #     att_emb, action_mask, self.grammar.action_emb.weight
        ...
        def apply_sketch(
            ...
            if isinstance(state, LSTMStateGold):
                ...
                assert isinstance(state, LSTMStatePred)
        ...
        def pass_detail_att_src(
            state: LSTMState, prev_tensor_dict: Dict[str, TensorPromiseOrTensor]
        ) -> Dict:
            h_state, _ = state.get_detail_state()
            src_encodings = state.get_src_encodings()
            aff_detail_src = state.get_affine_src()
            att_out = self.detail_dot_product_attention.forward_later(
                aff_detail_src, h_state, src_encodings
            )

            prev_tensor_dict.update({"att_out": att_out})
            return prev_tensor_dict

        def update_detail_att_src(
            state: LSTMState, prev_tensor_dict: Dict[str, TensorPromiseOrTensor]
        ) -> Dict:
            att_out = prev_tensor_dict["att_out"].result
            state.update_att_emb(att_out)
        ...
        def calc_detail_prob(
            ...
            if cur_symbol == "C":
                ...
                if self.use_column_pointer:
                    ...
                    mem_col_ids = [
                        action[1]
                        for action in state.get_action_history()
                        if action[0] == "C"
                    ]
                    col_appear_mask = torch.zeros(encoded_col.shape[0]).cuda()
                    col_appear_mask[mem_col_ids] = 1
                    col_probs = self.column_pointer_net.forward_later(
                        encoded_col, att_emb, col_appear_mask
                    )
                else:
                    col_probs = self.pointer_net(encoded_col, att_emb)

                col_probs = self.detail_column_similarity.forward_later(
                    state.get_detail_state()[0], encoded_col, None
                )

                prev_tensor_dict.update({"prob": col_probs})

            elif cur_symbol == "T":
                encoded_tab = state.get_tab_encodings()
                prev_action = state.get_prev_action()
                _, column_id = prev_action
                possible_table_ids = state.col_tab_dic[column_id]
                impossible_indices = [
                    idx
                    for idx in range(len(state.encoded_tab))
                    if idx not in possible_table_ids
                ]
                tab_probs = self.detail_table_similarity.forward_later(
                    state.get_detail_state()[0], encoded_tab, impossible_indices
                )

                # att_emb = state.get_att_emb()
                # tab_probs = self.table_pointer_net.forward_later(
                #     state.encoded_tab, att_emb, possible_table_ids
                # )
                ...
                action_mask[action_ids] = 0
                # att_emb = state.get_att_emb()
                # action_probs = self.action_linear_layer.forward_later(
                #     att_emb, action_mask, self.grammar.action_emb.weight
                # )
                impossible_indices = [
                    idx
                ...
                action_probs = self.detail_action_similarity.forward_later(
                    state.get_detail_state()[0],
                    self.grammar.action_emb.weight,
                    impossible_indices,
                )
        ...
        def apply_detail(
            ...
            if "prob" not in prev_tensor_dict:
                state.step()
                return {}
        ...
        states = SequentialMonad(states)(
            WhileLogic.While(state_class.sketch_not_done).Do(
