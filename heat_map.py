...
def draw_heat_map(nl_query, weights, title, path):
...
def draw_inference_score(actions, weights, title, path):
    assert len(actions) == len(weights), "length different {} {}".format(
        len(actions), len(weights)
    )
    # Parse
    used_actions = []
    used_weights = []
