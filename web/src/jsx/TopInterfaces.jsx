...
class TopInterfaces extends React.Component {
  ...
  render () {
    const classname = this.state.schema_showing ? "button_pretty showing-schema":"button_pretty hiding-schema"
    const e = db_ids.find (o => o.value === this.props.db_id);
    const db_img = e ? e.img : null
    const schema_div = this.state.schema_showing ? (<img className="schema-img" src={db_img} alt=""/>) : (<div className="schema_text">
    {' '}<b>Schema</b>{' '}
    </div>);
    return (
